...
Obviously if the value is other than 32, we wouldn't have the same number of bits as flags. If we need les flags, we could consider to use a `char` or a `short`. If we have more, a `long`.

Still, when the number of bits is not exactly the number of flags, this won't be as intuitive. 
...
The `&&` operand, will return `true` if **both sides** are `true`. So, once it knows that the left side is already `false`, it returns `false` without checking the right side. Thus, we only execute the call to `func0()` in the first if statement.
...
On this case, we might prefer using the `auto` instead but trying to go a step further on the program, we also propose to use the `std::function` class.
...
Also, in order to create the arrays properly, we defined a private  variable for the class: `size_t m_Size;`. It will be '0' until we call `GenerateData(int dataSize)`. Then, it will have the value of `dataSize`.
Or, it will get the value from another object if we are using the copy constructor.

#### Rule of 3
Since we added the copy constructor, we should define the default constructor, the destructor and the copy assignment operator.
In this program, the destructor is already defined and we don't use the assignment operator so we didn't defined it. See here an example of what it could be:
```
...
As we can see, the variable `m_Health` is an `unsigned int` so it will not represent negative values. The problem with the `GetHit(unsigned damage)` method will be that if the value of damage is higher than the current value of `m_Health`, it will assign to `m_Health` a negative value. What the value of `m_Health` will store is actually going to be a very big possitive value. 
...
```
  	...
  	funcCaller(&someB);
```
But,instead of seeing two calls to methods in A and then 2 calls to methods from B, we see that `funcCaller(&someB);` calls `A::baz()` despite being defined in B, `B::baz()`.
